...
async def on_chat_start():
    ...
    input_text = await ask_user_msg("Give the input text")
    content = input_text['output']
    if not await chk_inp.check_if_text_exists(content):
        table = await ontology.return_ontology(content)
        ...
        await chk_inp.put_into_directory(content)
        db_text = await v_db.create_embeddings_text(content)
    ...
    while True:
        ...
        ans = await qna.return_answer(ques['output'])
